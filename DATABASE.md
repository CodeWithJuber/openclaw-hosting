...
> **Agent 2** owns this file. Other agents import types from `packages/shared/src/types.ts`.
...
export const users = pgTable("users", {
  ...
  reseller_id: uuid("reseller_id").references(() => users.id), // for white-label
...
export const webhookLog = pgTable("webhook_log", {
  id: uuid("id").primaryKey().defaultRandom(),
  source: varchar("source", { length: 50 }).notNull(), // "whmcs", "hetzner", "instance"
...
CREATE INDEX idx_instances_whmcs_service_id ON instances(whmcs_service_id);
CREATE INDEX idx_metrics_instance_recorded ON metrics(instance_id, recorded_at DESC);
CREATE INDEX idx_ai_usage_instance_date ON ai_usage(instance_id, date DESC);
...
|-------|-----------|---------------|
| metrics | 90 days | Daily cron: DELETE WHERE recorded_at < NOW() - 90 days |
| ai_usage | 365 days | Monthly rollup to summary table |
| audit_log | 365 days | Archive to cold storage |
| webhook_log | 30 days | Daily cron delete |
| instances (terminated) | 30 days | Soft delete, hard delete after 30d |
